...
aws cloudformation describe-stacks --stack-name $1 --region $2 >/dev/null 2>&1 --profile $3
if [ $? -ne 0 ]
then
...
--stack-name $1 --region $2 --profile $3)

if [ $? -eq 0 ]
then
    echo "Waiting on $1 for delete completion..."
    aws cloudformation wait stack-delete-complete --stack-name $1 --region $2 --profile $3
    if [ $? -eq 0 ]
    then
